...
func (r *CredentialRepository) GetCredentialsByUserID(ctx context.Context, userID uuid.UUID) ([]*entities.Credential, error) {
	query, args, err := credentialsTable.SelectMany(
		[]string{
			...
			"sign_count", "is_active", "last_used_at", "last_login_ip",
			"client_info", "raw_registration_data", "created_at",
		},
		map[string]any{"user_id": userID, "is_active": true},
		nil,
	)
	if err != nil {
		return nil, err
...
func (r *CredentialRepository) DeactivateCredential(ctx context.Context, credentialID []byte, userID uuid.UUID) error {
	query, args, err := credentialsTable.Update(
		map[string]any{"is_active": false},
		map[string]any{"id": credentialID, "user_id": userID},
	)
	if err != nil {
		return err
	}
...
func (r *SessionRepository) SaveSession(ctx context.Context, session *entities.WebAuthnSession) error {
	...
	query, args, err := webauthnSessionsTable.Insert(map[string]any{
		...
		"ip_address":   nullIP(session.IPAddress),
...
func (r *SessionRepository) GetSessionByID(ctx context.Context, id uuid.UUID) (*entities.WebAuthnSession, error) {
	query, args, err := webauthnSessionsTable.SelectOne(
		[]string{"id", "user_id", "challenge", "type", "status", "session_data", "user_agent", "ip_address", "expires_at"},
		map[string]any{"id": id, "status": "PENDING"},
	)
	if err != nil {
		return nil, err
	}
...
func (r *SessionRepository) GetSessionByUserIDAndType(ctx context.Context, userID uuid.UUID, sessionType string) (*entities.WebAuthnSession, error) {
	query, args, err := webauthnSessionsTable.SelectOne(
		[]string{"id", "user_id", "challenge", "type", "status", "session_data", "user_agent", "ip_address", "expires_at"},
		map[string]any{"user_id": userID, "type": sessionType, "status": "PENDING"},
	)
	if err != nil {
		return nil, err
	}
...
func (r *SessionRepository) UpdateSessionStatus(ctx context.Context, id uuid.UUID, status string) error {
	query, args, err := webauthnSessionsTable.Update(map[string]any{"status": status}, map[string]any{"id": id})
	if err != nil {
		return err
	}
